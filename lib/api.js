...
function getCoordinates(address) {
  ...
  const apiUrl = 'https://wxs.ign.fr/essentiels/geoportail/geocodage/rest/0.1/search';
  const params = new URLSearchParams({
    q: searchAddress,
    format: 'json',
    limit: 1,
...
function reverseGeocode(lat, lng) {
  const apiUrl = 'https://wxs.ign.fr/essentiels/geoportail/geocodage/rest/0.1/reverse';
  const params = new URLSearchParams({
    lat: lat,
    lon: lng,
    format: 'json',
