const CONFIG = {
  ...
  BUFFER_TIME: 120,                   // Réduit à 2min pour prendre/déposer le vélo
  WALKING_SPEED: 1.4,                 // m/s (5 km/h)
  DECISION_THRESHOLD: 1.2,            // 20% plus rapide pour recommander
  VELIB_SPEED_FACTOR: 1.0             // Ajusté pour correspondre à Google Maps
};
...
// Fonction modifiée pour sélectionner les stations les plus proches sans changer la logique de temps
function getBestStations(stations, propertyToCheck, count = 3) {
  if (!stations || !Array.isArray(stations) || stations.length === 0) return [];
  
  // Filtrer d'abord pour assurer la disponibilité
  const availableStations = stations.filter(s => safeGet(s, propertyToCheck, 0) > 0);
  
  // Tri par proximité uniquement (temps de marche)
...
function analyzeVelibJourney(origin, destination) {
  ...
  .then(([departureStations, arrivalStations, bikeRoute, alternative]) => {
    ...
    // 7. Calculer le temps total en Vélib avec des estimations plus proches de Google Maps
const rawBikeTime = bikeRoute.duration;
// Utiliser un facteur de 1.0 au lieu de 0.75 pour mieux correspondre à Google Maps
const correctedBikeTime = Math.round(rawBikeTime * CONFIG.VELIB_SPEED_FACTOR); 
let velibTime = correctedBikeTime + 
                Math.min(departureStation.walkDuration, 300) + // Plafonner le temps de marche à 5 minutes max
                Math.min(arrivalStation.walkDuration, 300) +  // Plafonner le temps de marche à 5 minutes max
                CONFIG.BUFFER_TIME;

// Pour les courts trajets, réduire encore le buffer time
if (bikeRoute.distance < 2000) { // moins de 2km
  const shortTripBuffer = Math.min(CONFIG.BUFFER_TIME, 60); // Réduit à 1 minute max
