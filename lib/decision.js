const CONFIG = {
  ...
  BUFFER_TIME: 180,                   // 3min pour prendre/déposer le vélo (réduit de 5min)
  WALKING_SPEED: 1.4,                 // m/s (5 km/h - légèrement augmenté)
  DECISION_THRESHOLD: 1.2,            // 20% plus rapide pour recommander
  VELIB_SPEED_FACTOR: 0.75            // Facteur correctif pour le temps Vélib
};
...
function analyzeVelibJourney(origin, destination) {
  ...
  .then(([departureStations, arrivalStations, bikeRoute, alternative]) => {
    ...
    // 7. Calculer le temps total en Vélib (avec facteur correctif)
    const rawBikeTime = bikeRoute.duration;
    const correctedBikeTime = Math.round(rawBikeTime * (CONFIG.VELIB_SPEED_FACTOR || 0.75));
    let velibTime = correctedBikeTime + departureStation.walkDuration + 
                    arrivalStation.walkDuration + CONFIG.BUFFER_TIME;
    
    // Pour les courts trajets, réduire le buffer time
    if (bikeRoute.distance < 2000) { // moins de 2km
      const shortTripBuffer = Math.min(CONFIG.BUFFER_TIME, 120); // max 2min
      velibTime = correctedBikeTime + departureStation.walkDuration + 
                  arrivalStation.walkDuration + shortTripBuffer;
    }
